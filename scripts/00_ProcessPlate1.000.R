...
# Files to read in
platFile <- file.path(platDir, "SSF-2153.H7F35BGX2.unq.refseq.umi.dat")
...
info <- info[info$Plate == pNo,] # Subset for plate
...
png(AboxplotFile000, width = 1200, height = 600)
creatBoxPlots(eSetCPM[,pData(eSetCPM)$Experiment == "A"], title =  "Experiment A (log2CPM)")
dev.off()

png(BboxplotFile000, width = 1200, height = 600)
...
png(AbarplotFile000, width = 1200, height = 600)
creatBarPlots(eSet[,pData(eSet)$Experiment == "A"], title =  "Experiment A (Total Reads)")
dev.off()

png(BbarplotFile000, width = 1200, height = 600)
...
png(medVmadplotFile000, 400, 400)
...
png(clustMADplotFile000, width = 1000, height = 1000)
...
png(clustExpplotFile000, width = 1000, height = 1000)
MEDclust <- clusEset(eSetMED)
dev.off()

# Save results
eSetRem <- eSet
saveRDS(eSetRem, eSetRemFile000)
