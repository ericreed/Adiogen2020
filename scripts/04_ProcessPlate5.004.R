...
# Files to read in
platFile <- file.path(platDir, "SSF-12387.old.pipeline.plate1.HMCHHBGX3.unq.refseq.umi.dat")
...
png(AboxplotFile004, width = 1200, height = 600)
creatBoxPlots(eSetCPM[,pData(eSetCPM)$Experiment == "A"], title =  "Experiment A (log2CPM)")
dev.off()

png(BboxplotFile004, width = 1200, height = 600)
...
png(AbarplotFile004, width = 1200, height = 600)
creatBarPlots(eSet[,pData(eSet)$Experiment == "A"], title =  "Experiment A (Total Reads)")
dev.off()

png(BbarplotFile004, width = 1200, height = 600)
...
png(medVmadplotFile004, 400, 400)
...
png(clustMADplotFile004, width = 1000, height = 1000)
...
png(clustExpplotFile004, width = 1000, height = 1000)
MEDclust <- clusEset(eSetMED)
dev.off()

# Save
eSetRem <- eSet
saveRDS(eSetRem, eSetRemFile004)
