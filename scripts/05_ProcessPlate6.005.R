...
# Files to read in
platFile <- file.path(platDir, "SSF-12388_old_pipeline_Plate2_HMFMWBGX3.unq.refseq.umi.dat")
...
png(AboxplotFile005, width = 1200, height = 600)
creatBoxPlots(eSetCPM[,pData(eSetCPM)$Experiment == "A"], title =  "Experiment A (log2CPM)")
dev.off()

png(BboxplotFile005, width = 1200, height = 600)
...
png(AbarplotFile005, width = 1200, height = 600)
creatBarPlots(eSet[,pData(eSet)$Experiment == "A"], title =  "Experiment A (Total Reads)")
dev.off()

png(BbarplotFile005, width = 1200, height = 600)
...
png(medVmadplotFile005, 400, 400)
...
png(clustMADplotFile005, width = 1000, height = 1000)
...
png(clustExpplotFile005, width = 1000, height = 1000)
MEDclust <- clusEset(eSetMED)
dev.off()

# Save
eSetRem <- eSet
saveRDS(eSetRem, eSetRemFile005)
