...
# Files to read in
platDir <- file.path(datDir, "bulk", pID)
platFile <- file.path(platDir, "SSF-2154.H7HKKBGX2.unq.refseq.umi.dat")
...
png(AboxplotFile001, width = 1200, height = 600)
creatBoxPlots(eSetCPM[,pData(eSetCPM)$Experiment == "A"], title =  "Experiment A (log2CPM)")
dev.off()

png(BboxplotFile001, width = 1200, height = 600)
...
png(AbarplotFile001, width = 1200, height = 600)
creatBarPlots(eSet[,pData(eSet)$Experiment == "A"], title =  "Experiment A (Total Reads)")
dev.off()

png(BbarplotFile001, width = 1200, height = 600)
...
png(medVmadplotFile001, 400, 400)
...
png(clustMADplotFile001, width = 1000, height = 1000)
...
png(clustExpplotFile001, width = 1000, height = 1000)
MEDclust <- clusEset(eSetMED)
dev.off()

# Save
eSetRem <- eSet
saveRDS(eSetRem, eSetRemFile001)
